...
const standardComponents = [
  'Button', 'Skeleton', 'Avatar', 'Sheet', 'Input', 'Textarea', 'PasswordInput',
  'Select', 'DropdownMenu', 'Section', 'SearchModeSelector', 'ChatMenuItem',
  'Carousel', 'CarouselPrevious', 'CarouselNext', 'CarouselContent', 'CarouselItem'
]
...
for (const file of files) {
  ...
  for (const comp of standardComponents) {
    const regex1 = new RegExp(`(<${comp}[^>]*?)\\bwidth={([^}]+)}\\s*height={\\2}`, 'g')
    const regex2 = new RegExp(`(<${comp}[^>]*?)\\bwidth="([^"]+)"\\s*height="\\2"`, 'g')
