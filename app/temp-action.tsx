...
async function submit(formData?: FormData, skip?: boolean) {
  ...
  const chatId = aiState.get().length > 0 ? aiState.get()[0].chatId : nanoid()
  ...
  async function processEvents() {
    ...
    if (useSpecificAPI && answer.length === 0) {
      ...
      const modifiedMessages = aiState.get().map(msg =>
        msg.role === 'function'
          ? {
              ...msg,
...
// Note: In the case of { chatid, messages }, nested object values cause errors

// so we need to flatten the object
