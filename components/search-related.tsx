      ...
      {data?.items
        ...
        .map((item, index) => (
          <div className="w-full" key={index}>
            <Button
              variant="link"
              className="w-fit justify-start px-0 py-1 h-fit font-semibold text-accent-foreground/50 whitespace-normal text-left"
              type="submit"
              name={'related_query'}
              value={item?.query}
            >
              <ArrowRight className="h-4 w-4 mr-2 text-accent-foreground/50" />
              {item?.query}
            </Button>
          </div>
        ))}
